import { getQuery, ParsedQuery, parseQuery } from "ufo"
...
export function createInstance(options: Options = {}) {
    ...
    return (url: URL | string, options: Options = { driver }) => toRawSQL(url, options)
}

/**
 * Turns a URL into a raw SQL string.
 */
export function toRawSQL(url: URL | string, options: Options = {}) {
    const driver = options.driver || sqlite(options.schema)
 ...
 */
export function toQueryParams(url: URL | string, limit = 0): QueryParams {
